node {
  ...
  stage ('Deliver & Deployment') {
      sh 'curl -u admin:redhat@123 -T target/**.war "http://65.0.95.46:8080/:8080/manager/text/deploy?path=/malli&update=true"'
  }
  stage ('SmokeTest') {
      sh 'curl --retry-delay 10 --retry 5 "http://65.0.95.46:8080/malli"'
  }
}
